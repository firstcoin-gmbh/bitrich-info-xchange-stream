...
public class CexioWebSocketOrderbookUpdate extends CexioWebSocketOrderbookTransaction {
    ...
    public CexioWebSocketOrderbookUpdate(@JsonProperty(PROP_ID) long id, 
            @JsonProperty(PROP_PAIR) String pair, @JsonProperty(PROP_BIDS) BigDecimal[][] bids, 
            @JsonProperty(PROP_ASKS) BigDecimal[][] asks, @JsonProperty(PROP_TIME) long time) {
    ...
    public CexioOrderbook toCexioOrderBook(CexioOrderbook orderbook) {
        orderbook.updateData(time / 1000L, getBids(), getAsks());
        return orderbook;
    }
