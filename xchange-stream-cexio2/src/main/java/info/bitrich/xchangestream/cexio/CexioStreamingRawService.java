...
public class CexioStreamingRawService extends JsonNettyStreamingService {
      ...
      if (dataNode != null) {
        ...
        if (pairNode != null) {
          ...
          throw new IOException(
              String.format("No \"pair\" node found in message \"%s\"!", message));
        }
      }
    }
    if (result == null) {
  ...
  public String getSubscribeMessage(String channelName, Object... args) throws IOException {
    if (channelName.startsWith(CHANNEL_ORDERBOOK)) {
      ...
      if (args.length == 1) {
        ...
        subscription =
            new CexioWebSocketOrderbookSubscription(
                ((String) args[0]).split(":"), Integer.parseInt(((String) args[1])));
      } else {
        throw new IllegalArgumentException("One currency pair and one optional depth expected!");
      }
      CexioWebSocketSubscriptionMessage message =
          new CexioWebSocketSubscriptionMessage(ORDERBOOK_SUBSCRIPTION_EVENT, subscription);

      return objectMapper.writeValueAsString(message);
    } else {
      throw new IllegalArgumentException(
          String.format(
              "Cannot get subscription data for unknown channel name \"%s\"", channelName));
    }
  }
  ...
  public String getUnsubscribeMessage(String channelName) throws IOException {
    ...
    if (channelName.startsWith(CHANNEL_ORDERBOOK)) {
      ...
      final CexioWebSocketSubscriptionMessage message =
          new CexioWebSocketSubscriptionMessage(
