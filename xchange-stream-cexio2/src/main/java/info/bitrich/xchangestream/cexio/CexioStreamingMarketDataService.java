...
public class CexioStreamingMarketDataService implements StreamingMarketDataService {
  ...
  public Observable<OrderBook> getOrderBook(CurrencyPair currencyPair, Object... args) {
    ...
    Observable<Optional<CexioWebSocketOrderbookTransaction>> subscribedChannel =
        service
            .subscribeChannel(channelName, pair, depth)
            .map(
                jsonNode -> {
                  ...
                  if (ORDERBOOK_SUBSCRIPTION_EVENT.equalsIgnoreCase(eventName)) {
                    return Optional.of(
                        mapper.treeToValue(dataNode, CexioWebSocketOrderbookSnapshot.class));
                  } else if (MARKET_DATA_UPDATE_EVENT.equalsIgnoreCase(eventName)) {
                    return Optional.of(
                        mapper.treeToValue(dataNode, CexioWebSocketOrderbookUpdate.class));
                  } else {
