...
class BitfinexStreamingMarketDataService implements StreamingMarketDataService {
  ...
  private static final ChecksumBigDecimalToStringConverter CHECKSUM_CONVERTER =
      new ChecksumBigDecimalToStringConverter();
  ...
  public Observable<OrderBook> getOrderBook(CurrencyPair currencyPair, Object... args) {
    ...
    final Observable<ImmutableTriple<BitfinexOrderbook, Integer, OrderBook>> subscribedChannel =
        service
            .subscribeChannel(channelName, new Object[] {pair, "P0", depth})
            .observeOn(Schedulers.computation())
            .scan(
                ImmutableTriple.of(
                    new BitfinexOrderbook(new BitfinexOrderbookLevel[0]),
                    (Integer) null,
                    (OrderBook) null),
                (immutableTriple, jsonNode) -> {
                  if ("cs".equals(jsonNode.get(1).asText())) {
                    return ImmutableTriple.of(
                        ...
                        jsonNode.get(2).asInt(),
                        immutableTriple.getRight());
                  } else {
                    final BitfinexOrderbook bitfinexOrderbook =
                        (jsonNode.get(1).get(0).isArray()
                                ? mapper.treeToValue(
                                    jsonNode, BitfinexWebSocketSnapshotOrderbook.class)
                                : mapper.treeToValue(
                                    jsonNode, BitfinexWebSocketUpdateOrderbook.class))
    ...
    final Disposable checksumDisposable =
        subscribedChannel
