...
public class KrakenOrderBookUtils {
    ...
    public static KrakenOrderBook parse(List<?> jsonParseResult) {
        try {
            ...
            KrakenOrderBookMessageType orderBookType = orderBookItemsMap.keySet().stream()
                    .anyMatch(key -> StringUtils.equalsAny(key, ASK_SNAPSHOT, BID_SNAPSHOT)) ? KrakenOrderBookMessageType.SNAPSHOT : KrakenOrderBookMessageType.UPDATE;
    ...
    public static KrakenPublicOrder[] getItemsArray(List<List<String>> values) {
        return values == null ? new KrakenPublicOrder[0] : values.stream().map(KrakenOrderBookUtils::extractKrakenPublicOrder).toArray(KrakenPublicOrder[]::new);
    }
    ...
    public static <T> T getTypedValue(Iterator<?> iterator, Class<T> clazz, String fieldName) throws KrakenException {
        ...
        if (!clazz.isAssignableFrom(object.getClass())) {
            throw new KrakenException(String.format("Expected value of %s type for %s filed but there is invalid type %s", clazz, fieldName, object.getClass().getName()));
        }
        return (T) object;
    }
