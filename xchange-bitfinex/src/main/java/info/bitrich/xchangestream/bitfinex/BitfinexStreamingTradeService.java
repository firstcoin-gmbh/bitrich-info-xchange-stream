...
public class BitfinexStreamingTradeService implements StreamingTradeService {
    ...
    @Override
    public Observable<UserTrade> getUserTrades(Object... args) {
        return getRawAuthenticatedTrades()
                .flatMapIterable(trade -> {
                    UserTrades adaptedUserTrades = adaptTradeHistory(
                            new BitfinexTradeResponse[] {
                                    adaptTradeResponse(trade)
                            }, 
    ...
    @Override
    public Observable<LimitOrder> getOpenOrders(Object... args) {
        return getRawAuthenticatedOrders()
                .flatMapIterable(order -> {
                    OpenOrders adaptedOpenOrders = adaptOrders(new BitfinexOrderStatusResponse[] {
                            adaptOrderStatusResponse(order)
                    });
    ...
    public void placeLimitOrder(LimitOrder limitOrder) {
        ...
        final BitfinexWebSocketOrderTransaction<BitfinexWebSocketNewOrder> transaction = new BitfinexWebSocketOrderTransaction<>(
                AUTH_CHANNEL_ID, "on", null, newOrder);
    ...
    public void cancelOrder(String orderId) {
        final BitfinexWebSocketCancelOrder cancelOrder = new BitfinexWebSocketCancelOrder(new BigInteger(orderId));
        final BitfinexWebSocketOrderTransaction<BitfinexWebSocketCancelOrder> transaction = new BitfinexWebSocketOrderTransaction<>(
                AUTH_CHANNEL_ID, "oc", null, cancelOrder);
        final ObjectMapper mapper = StreamingObjectMapperHelper.getObjectMapper();
        
        try {
            sendMessage(mapper.writeValueAsString(transaction));
        } catch (JsonProcessingException e) {
            throw new ExchangeException(String.format("Could not cancel order with id \"%s\"", orderId), e);
        }
    }
    ...
    private <T> Observable<T> withAuthenticatedService(Function<BitfinexStreamingPrivateService, Observable<T>> serviceConsumer) {
